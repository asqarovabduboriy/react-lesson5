...
function Navbar(props) {
    const ul_item = ["Shop", "AI", "Mobile", "Tv & audio", "Appliances", "Computing", "Displays", "Accessories", " SmartThings"];

    let items_navbar = ul_item?.map((el, inx) => (
        <li className='li_items_navbar' key={inx}>
            {el}
        </li>
    ))

    const nav_ul_item = ["Explore", "Support", " For Business"];

    let nav_li_item = nav_ul_item?.map((el, inx) => (
        <li className='li_items_navbar' key={inx}>
            {el}
        </li>
    ))
    ...
    return (
        <><header>
            <div className='container' >
                <div className="big_wrapprer_header">
                    <div className='logo'>
                        <a href="https://www.samsung.com/us/"><svg className="icon_samsung" xmlns="http://www.w3.org/2000/svg" width="120" height="32" viewBox="0 0 120 32" role="presentation" aria-hidden="true" >
                    ...
                    <nav>
                        <ul className='navbar_ul_item'>
                            {items_navbar}
                        </ul>
                        <ul className='nav_ul_item_two'>
                            {nav_li_item}
                        </ul>
                    </nav>
                    <div className='icon_container'>
                        <IoIosSearch className='icon_nav' />
                        <BsCart2 className='icon_nav' />
                        <FaRegUser className='icon_nav' />
                    </div>
                </div>
            </div >
        </header>
